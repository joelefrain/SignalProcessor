...
from signalprocessor.processing import CorrectionConfig, apply_iir_filter, correct_record, polynomial_baseline
...
def test_polynomial_baseline_constrains_velocity_and_displacement_discretely():
    dt = 0.01
    time = np.arange(0.0, 20.0, dt)
    acc = 0.01 + 0.001 * time + 0.15 * np.sin(2.0 * np.pi * 1.0 * time) * np.exp(-0.05 * time)
...
def test_supported_iir_filter_families_return_finite_arrays():
    ...
    for filter_type in ("butterworth", "cheby1", "cheby2", "ellip", "bessel"):
        ...
        assert meta["filter_type"] in {"butterworth", "cheby1", "cheby2", "ellip", "bessel"}
